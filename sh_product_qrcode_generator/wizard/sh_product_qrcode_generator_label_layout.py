...
class ShProductQRCodeGeneratorLabelLayout(models.TransientModel):
    ...
    def _compute_dimensions(self):
        for wizard in self:
            ...
            # special handling
            if wizard.print_format == 'dymo':
                continue
            elif wizard.print_format == 'bap_2x7':
                wizard.columns = 2
                wizard.rows = 7
                continue

            # normal parse for '2x7xprice', '4x12', etc.
            try:
                parts = wizard.print_format.replace('xprice', '').split('x')
                if len(parts) >= 2 and parts[0].isdigit() and parts[1].isdigit():
                    wizard.columns = int(parts[0])
                    wizard.rows = int(parts[1])
            except Exception:
    ...
    def _prepare_report_data(self):
        ...
        elif self.print_format == 'bap_2x7':
            xml_id = 'sh_product_qrcode_generator.action_bap_control_label_2x7'
    ...
    def process_with_qr_code(self):
        ...
        if not xml_id:
            raise UserError(
                _('Unable to find report template for format: %s') % (self.print_format,)
            )
